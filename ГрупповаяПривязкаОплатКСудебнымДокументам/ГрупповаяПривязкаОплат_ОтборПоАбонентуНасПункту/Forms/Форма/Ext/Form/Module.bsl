...
Процедура ОбновитьСостояниеНадписиПериода()
	...
	Если Не ЗначениеЗаполнено(Объект.НачалоПериодаОплат) или Не ЗначениеЗаполнено(Объект.ОкончаниеПериодаОплат) Тогда
		СтрЗаголококНадписи = "Не указан период!";
		//ЭлементыФормы.НадписьПредставлениеПериода.Заголовок = СтрЗаголококНадписи;
		//ЭлементыФормы.НадписьПредставлениеПериода.ЦветТекста = WebЦвета.Красный;
	Иначе
		...
		Иначе 
			...
			//ЭлементыФормы.НадписьПредставлениеПериода.Заголовок = СтрЗаголококНадписи;
			//ЭлементыФормы.НадписьПредставлениеПериода.ЦветТекста = WebЦвета.Черный;
		КонецЕсли;	
